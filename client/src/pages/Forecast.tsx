...
export default function Forecast() {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/forecast/${selectedClient}/historical`,
      );
  ...
  const { data: sentimentPredictions = [], refetch: refetchSentiment } =
    useQuery<ForecastPrediction[]>({
      queryKey: ["/api/forecast", selectedClient, "predictions"],
      queryFn: async () => {
        const response = await fetch(
          `/api/forecast/${selectedClient}/predictions?type=sentiment`,
        );
        if (!response.ok)
          throw new Error("Failed to fetch sentiment predictions");
        return response.json();
      },
      enabled: !!selectedClient,
    });
  ...
  const { data: churnPredictions = [], refetch: refetchChurn } = useQuery<
    ForecastPrediction[]
  >({
    queryKey: ["/api/forecast", selectedClient, "churn_predictions"],
    queryFn: async () => {
      const response = await fetch(
        `/api/forecast/${selectedClient}/predictions?type=churn_risk`,
      );
  ...
  const generateSampleDataMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/forecast/sample-data", {
        method: "POST",
      });
    ...
    onSuccess: () => {
      ...
      if (selectedClient) {
        queryClient.invalidateQueries({
          queryKey: ["/api/forecast", selectedClient],
        });
      }
    },
    onError: () => {
  ...
  const prepareSentimentChartData = () => {
    ...
    return [...recentHistorical, ...futurePredictions].sort(
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          ...
          >
            Client Forecast Analytics
          </h1>
          <p className="text-muted-foreground mt-2">
            AI-powered sentiment and churn risk predictions using historical
            client data
          </p>
        </div>
      ...
      {clientsLoading ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Choose a client to view their forecast analytics
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Select
              ...
              <SelectContent>
                {clients.map((client) => (
                  <SelectItem key={client.client_id} value={client.client_id}>
                    Client {client.client_id} - {client.primary_contact} (
                    {client.region})
                  </SelectItem>
                ))}
              </SelectContent>
                    ...
                    <Card>
                      <CardHeader className="flex flex-row items-center justify-between">
                        <div>
                          ...
                          <CardDescription>
                            AI-powered 6-month future sentiment trend
                            predictions using GPT-4 Turbo
                          </CardDescription>
                        </div>
                        <div className="flex items-center gap-2">
                          {sentimentPredictions.length > 0 && (
                            <Badge variant="outline" className="text-green-600">
                              {(sentimentPredictions[0]?.confidence || 0) * 100}
                              % Confidence
                            </Badge>
                          )}
                          <Button
                      ...
                      <CardContent>
                        {historicalLoading ? (
                          ...
                          <div className="h-80 flex flex-col items-center justify-center text-muted-foreground">
                            ...
                            <p>
                              No data available. Generate sample data to get
                              started.
                            </p>
                          </div>
                        ) : (
                          <ResponsiveContainer width="100%" height={400}>
                            <LineChart data={sentimentChartData}>
                              ...
                              <YAxis
                                ...
                                label={{
                                  value: "Sentiment Score", // 👈 Y-axis legend text
                    ...
                    <Card>
                      <CardHeader className="flex flex-row items-center justify-between">
                        <div>
                          ...
                          <CardDescription>
                            AI-powered 2-quarter future churn risk predictions
                            using GPT-4 Turbo
                          </CardDescription>
                        </div>
                        <div className="flex items-center gap-2">
                          {churnPredictions.length > 0 && (
                            <Badge variant="outline" className="text-red-600">
                              {(churnPredictions[0]?.confidence || 0) * 100}%
                              Confidence
                            </Badge>
                          )}
                          <Button
                            ...
                            disabled={
                              forecastLoading.churn || !historicalData.length
                            }
                ...
                {(sentimentInsight || churnInsight) && (
                  <Card className="border-l-4 border-l-blue-500">
                    <CardHeader>
                      ...
                      <CardDescription>
                        OpenAI GPT-4 analysis based on {historicalData.length}{" "}
                        data points from{" "}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-start gap-2 mb-2">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {(() => {
                                      ...
                                      const values = sentimentData.map(
                                        (d) => d.sentiment_score,
                                      );
                                      const avg = (
                                      ...
                                      const min = Math.min(...values).toFixed(
                                        3,
                                      );
                                      const max = Math.max(...values).toFixed(
                                        3,
                                      );
                                      const latest =
                                        sentimentData[
                                      ...
                                      return (
                                        <div className="text-xs text-blue-700 dark:text-blue-300 space-y-1">
                                          <p>
                                            ...
                                            {sentimentData.length} data points
                                          </p>
                                          <p>
                                            <strong>Range:</strong> {min} to{" "}
                                            {max} • <strong>Average:</strong>{" "}
                                            {avg}
                                          </p>
                                          <p>
                                            <strong>Latest Value:</strong>{" "}
                                            {latest} • <strong>Trend:</strong>{" "}
                                            {Number(latest) > Number(avg)
                                              ? "Above Average ↗"
                                              : "Below Average ↘"}
                                          </p>
                                        </div>
                              ...
                              <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg">
                                <div className="flex items-start gap-2 mb-2">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {(() => {
                                      const churnData = historicalData.filter(
                                        (d) =>
                                          typeof d.churn_probability ===
                                          "number",
                                      );
                                      ...
                                      const values = churnData.map(
                                        (d) => d.churn_probability,
                                      );
                                      const avg = (
                                        (values.reduce((a, b) => a + b, 0) /
                                          values.length) *
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                                ...
                                <div className="bg-red-50 dark:bg-red-900/20 p-2 rounded">
                                  ...
                                  {(() => {
                                    const issueData = historicalData.filter(
                                      (d) => typeof d.issue_count === "number",
                                    );
                                ...
                                <div className="bg-yellow-50 dark:bg-yellow-900/20 p-2 rounded">
                                  ...
                                  {(() => {
                                    ...
                                    const total = escalationData.reduce(
                                      (a, b) => a + b.escalation_count,
                                      0,
                                    );
                                    return (
                                      <p className="text-yellow-700 dark:text-yellow-300">
                                  ...
                                  <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-xs text-blue-700 dark:text-blue-300">
                                    ...
                                    {(sentimentPredictions.length > 0 ||
                                      ...
                                      <span className="ml-2">
                                        ...
                                        {sentimentPredictions.length > 0 &&
                                          ` Sentiment: ${((sentimentPredictions[sentimentPredictions.length - 1]?.confidence ?? 0) * 100).toFixed(0)}%`}
                                        {churnPredictions.length > 0 &&
                                          ` Churn: ${((churnPredictions[churnPredictions.length - 1]?.confidence ?? 0) * 100).toFixed(0)}%`}
                                      </span>
                                    )}
                                  </div>
                                );
